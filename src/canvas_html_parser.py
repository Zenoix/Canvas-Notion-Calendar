...
class CanvasToNotionHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        if (
            len(self.__tag_stack) > 0
            and self.__tag_stack[-1] == "p"
            and self.__latest_block
        ):
    ...
    def handle_endtag(self, tag):
        if tag in self.__valid_tags:
            ...
            self.__latest_list_type = None
        if len(self.__tag_stack) == 0 and self.__latest_block:
            self.__output.append(self.__latest_block)
            self.__latest_list_type = None
            self.__latest_block = None

    def handle_data(self, data):
        if len(self.__tag_stack) == 0 or self.__unknown_tag:
            return
        elif len(self.__tag_stack) == 1:
            self.__new_block_data(data)
        else:
            self.__inline_data(data)

    def __new_block_data(self, data):
        latest_opening_tag = self.__tag_stack[0]
        if latest_opening_tag in ("p", "span", "a", "strong", "em"):
            self.__latest_text_type = "paragraph"
        elif latest_opening_tag in ("h1", "h2", "h3"):
            self.__latest_text_type = f"heading_{latest_opening_tag[-1]}"
            ...
            self.__latest_block = {"object": "block", "type": "divider", "divider": {}}
            return
        ...
        if latest_opening_tag == "a":
            self.__latest_block[self.__latest_text_type]["rich_text"][0]["text"][
                "link"
            ] = {"url": self.__latest_url}
        elif latest_opening_tag == "strong":
            self.__latest_block[self.__latest_text_type]["rich_text"][0][
                "annotations"
            ] = {"bold": True}
    ...
    def __inline_data(self, data):
        latest_opening_tag = self.__tag_stack[-1]

        if latest_opening_tag == "a":
            self.__latest_block[self.__latest_text_type]["rich_text"].append(
                {
                    "type": "text",
                    "text": {"content": data, "link": {"url": self.__latest_url}},
                }
            )
        elif latest_opening_tag == "strong":
            self.__latest_block[self.__latest_text_type]["rich_text"].append(
...
if __name__ == "__main__":
    ...
    parser = MyHTMLParser()
    parser.feed(
        "<ul><li>hello</li><li>testing</li><li><strong>bold text</strong></li></ul>"
    )
    parser.close()

    parser = CanvasToNotionHTMLParser()
    parser.feed(
        '<ul><li>hello</li><li>testing</li><li>bold text</li></ul><a href="www.google.com">hello</a>'
    )
    content = parser.parsed_content
    parser.close()
    print(json.dumps(content, indent=2))
